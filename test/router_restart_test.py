...
sftp.close()
print(run("chmod 755 /etc/init.d/dockermanager && /etc/init.d/dockermanager enable && /etc/init.d/dockermanager restart"))
time.sleep(4)
print("status:", run("/etc/init.d/dockermanager status"))
print("进程:", run("ps | grep docker-manager | grep -v grep | head -2"))
...
if not TOKEN.startswith("dm_session"):
    print("!! 失败:", run("logread | grep -i 'dockermanager\\|docker-manager' | tail -8"))
else:
